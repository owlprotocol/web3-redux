...
describe(`${name}/sagas/updateDB.ts`, () => {
    const itemInitial: Network = validate({ networkId, name: 'test1' });
    const item: Network = validate({ networkId, name: 'test2' });
    let db: Connector;
    ...
    describe('unit', () => {
        ...
        it('updateDBSaga', async () => {
            ...
            testSaga(updateDBSaga, updateDBAction(item))
                ...
                .call([db, db.connect])
                .next(models)
                .call([models[name], models[name].save], item.networkId, item, true)
                .next()
                .isDone();
        });
    });
