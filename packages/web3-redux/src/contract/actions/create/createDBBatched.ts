import { createAction } from '@reduxjs/toolkit';
import { v4 as uuidv4 } from 'uuid';
import { omit } from '../../../utils/lodash/index.js';
import { name } from '../../common.js';
import { Contract, validate } from '../../model/index.js';

export const CREATE_DB_BATCHED = `${name}/CREATE_DB_BATCHED`;
export const createDBBatchedAction = createAction(CREATE_DB_BATCHED, (payload: Contract[], uuid?: string) => {
    return {
        payload: payload.map(validate).map((p) => omit(p, ['web3Contract', 'web3SenderContract'])),
        meta: {
            uuid: uuid ?? uuidv4(),
        },
    };
});

export type CreateDBBatchedAction = ReturnType<typeof createDBBatchedAction>;
export const isCreateDBBatchedAction = createDBBatchedAction.match;

export default createDBBatchedAction;
