...
import selectERC1155TokenIds from './selectERC721TokenIds.js';
...
export const selectERC1155TokenUris = (state: any, networkId: string | undefined, address: string | undefined) => {
    ...
    const tokendIdData = selectERC1155TokenIds(state, networkId, addressChecksum);
    const tokenIds = tokendIdData ? tokendIdData[0] : [];
    if (tokenIds.length == 0) return [];

    //Programmatic method - baseURI + tokenId
    const tokenId0 = tokenIds[0];
    const tokenUri0 = selectContractCall(state, { networkId, address: addressChecksum }, 'uri', {
        args: [tokenId0],
    }) as string | undefined;
    ...
    const tokenUris = tokenIds.map((tokenId) => {
        return selectContractCall(state, { networkId, address: addressChecksum }, 'uri', {
            args: [tokenId],
        }) as string | undefined;
    });
...
export default selectERC1155TokenUris;
